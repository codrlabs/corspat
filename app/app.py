...
def alert_center():
    ...
    cur = db.cursor()
    data = cur.execute("SELECT * FROM alerts ORDER BY alert_id DESC LIMIT 3").fetchall()
...
def path():
    if request.method == "POST":
        ...
        # Query add link
        cur = db.cursor()
        cur.execute("INSERT INTO path (user_id, course_uuid, course_domain, course_url, course_title, course_desc, course_img, finished, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (session["user_id"], str(uuid.uuid1()), data['hostname'], data['url'], data['title'], data['desc'], data['img'], 0, get_now(),)).fetchone()
...
def delete_course(uuid):
    ...
    cur = db.cursor()
    title_deleted = cur.execute("SELECT course_title FROM path WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],)).fetchone()
    cur.execute("DELETE FROM path WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],))
...
def toggle_finish(uuid):
    ...
    cur = db.cursor()
    curr_progression = cur.execute("SELECT finished FROM path WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],)).fetchone()
    selected_course = cur.execute("SELECT course_title FROM path WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],)).fetchone()

    # Check if finished or not
    if not curr_progression[0]:
        cur.execute("UPDATE path SET finished = 1 WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],))
        flash(f'Congratulation on finishing: {selected_course[0].strip()}.')
    else:
        cur.execute("UPDATE path SET finished = 0 WHERE course_uuid = ? AND user_id = ?", (uuid, session["user_id"],))
...
def timetracker():
    ...
    # Get current courses
    cur = db.cursor()
    current_courses = cur.execute("SELECT course_uuid, course_title FROM path WHERE user_id = ? AND finished = 0", (session['user_id'],)).fetchall()

    # Get DESC history
    timetrack_history = cur.execute("SELECT * FROM timetracker WHERE user_id = ? ORDER BY started_at DESC", (session['user_id'],)).fetchall()
...
def update_tt():
    ...
    cur = db.cursor()
    get_data = cur.execute("SELECT * FROM path WHERE user_id = ? AND course_uuid = ?", (session['user_id'], request.form.get("this_course"),)).fetchone()

    # Return last row to check
    check_last = cur.execute("SELECT * FROM timetracker WHERE user_id = ? ORDER BY started_at DESC LIMIT 1", (session['user_id'],)).fetchone()
    check_count = cur.execute("SELECT COUNT(*) FROM timetracker WHERE user_id = ?", (session['user_id'],)).fetchone()

    # Timetracker
    if int(check_count[0]) == 0:
        # New history if empty
        cur.execute("INSERT INTO timetracker (user_id, course_uuid, course_domain, course_title, started_at, track_uuid) VALUES (?, ?, ?, ?, ?, ?)", (session['user_id'], get_data[1], get_data[2], get_data[4], get_now(), str(uuid.uuid1())))
...
def settings():
    ...
    # Get account details
    cur = db.cursor()
    user_info = cur.execute("SELECT * FROM users WHERE id = ?;", (session['user_id'],)).fetchone()
    ...
    if request.method == "POST":
        ...
        checkpassword = bcrypt.checkpw(passwd.encode("utf-8"), user_info[2])
        if not checkpassword:
            flash("Invalid password.")
            return render_template('settings.html', user_info=user_info)

        elif fname and lname and email:
            cur.execute("UPDATE users SET email = ?, first_name = ?, last_name = ? WHERE id = ?;", (email, fname, lname, session['user_id'],))
            db.commit()
            user_info = cur.execute("SELECT * FROM users WHERE id = ?;", (session['user_id'],)).fetchone()
            ...
            if new_pass == new_pass_verify:
                cur.execute("UPDATE users SET hash = ? WHERE id = ?;", (bcrypt.hashpw(new_pass.encode("utf-8"), bcrypt.gensalt()), session['user_id'],))
                db.commit()
            ...
            if check_confirmation == 'Reset':
                ...
                cur.execute("DELETE FROM timetracker WHERE user_id = ?;", (session['user_id'],))
                cur.execute("DELETE FROM path WHERE user_id = ?;", (session['user_id'],))
                db.commit()
...
def download():
    ...
    if request.method == 'POST':
        ...
        data_rows = cur.execute("SELECT course_domain, course_title, started_at, finished_at, duration FROM timetracker WHERE user_id = ?", (session["user_id"],)).fetchall()
...
def login():
    ...
    if request.method == "POST":
        ...
        cur = db.cursor()
        query = cur.execute("SELECT * FROM users WHERE email = ?", (request.form.get("email").lower(),)).fetchone()
...
def register():
    ...
    if request.method == "POST":
        ...
        cur = db.cursor()
        result = db.execute("SELECT email FROM users WHERE email = ?", (request.form.get("email").lower(),)).fetchone()
